...
void vk_create_vertex_buffer(VkPhysicalDevice vk_physical_device, VkDevice vk_device) {
    ...
    if (vkCreateBuffer(vk_device, &bufferInfo, nullptr, &vertexBuffer) != VK_SUCCESS) {
    ...
    if (vkAllocateMemory(vk_device, &allocInfo, nullptr, &vertexBufferMemory) != VK_SUCCESS) {
        throw std::runtime_error("failed to allocate vertex buffer memory!");
    }

    vkBindBufferMemory(vk_device, vertexBuffer, vertexBufferMemory, 0);

    void* data;
    vkMapMemory(vk_device, vertexBufferMemory, 0, bufferInfo.size, 0, &data);
